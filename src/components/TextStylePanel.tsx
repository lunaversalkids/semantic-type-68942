...
export const TextStylePanel = ({
  ...
  const [baselineMode, setBaselineMode] = useState<'normal' | 'superscript' | 'subscript'>('normal');
  ...
  const handleBaselineCycle = () => {
    ...
    if (baselineMode === 'normal') {
      ...
      // Deactivate superscript, activate subscript
      editor.chain().focus().toggleSuperscript().run();
      editor.chain().focus().toggleSubscript().run();
      setBaselineMode('subscript');
    } else {
      // Deactivate subscript, return to normal
      editor.chain().focus().toggleSubscript().run();
